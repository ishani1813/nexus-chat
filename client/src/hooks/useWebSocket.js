...
export function useWebSocket({ onMessage, onMetrics }) {
  const wsRef          = useRef(null);
  const retriesRef     = useRef(0);
  const retryTimer     = useRef(null);
  const pingTimer      = useRef(null);
  const pendingAcks    = useRef(new Map()); // clientMsgId → { resolve, ts }
    ...
    pingTimer.current = setInterval(() => {
      ...
      const handler = (ev) => {
        try {
          ...
          setRtt(Date.now() - t0);
          if (msg.metrics) onMetricsRef.current?.(msg.metrics);
   ...
   */
  const sendMessage = useCallback((content, clientMsgId) => {
    return new Promise((resolve) => {
      pendingAcks.current.set(clientMsgId, { resolve, ts: Date.now() });
      const ok = rawSend({ type: "message", content, clientMsgId });
      if (!ok) { pendingAcks.current.delete(clientMsgId); resolve(null); return; }
      setTimeout(() => {
  ...
  const join       = useCallback((username, room) => rawSend({ type: "join", username, room }), [rawSend]);
  const switchRoom = useCallback((room) => rawSend({ type: "switch_room", room }), [rawSend]);
  const sendTyping = useCallback((isTyping) => rawSend({ type: "typing", isTyping }), [rawSend]);
