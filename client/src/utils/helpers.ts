...
export function avatarColor(username = ""): AvatarPalette {
  let h = 0;
...
// ── Message grouping ──────────────────────────────────────────────────────────
export function shouldGroup(prev: ChatMessage | undefined, cur: ChatMessage): boolean {
  if (!prev || prev.type === "system" || cur.type === "system") return false;
  if (prev.userId !== cur.userId) return false;
  return new Date(cur.timestamp).getTime() - new Date(prev.timestamp).getTime() < 60_000;
}
...
export const ROOM_META: Record<RoomId, { icon: string; desc: string }> = {
